...
class Arachnid extends Events {
    constructor(domain: any) {
        ...
        // @ts-expect-error ts-migrate(2322) FIXME: Type 'boolean' is not assignable to type '() => th... Remove this comment to see the full error message
     ...
     */
    setMaxResultsNum(maxResultsNum: any) {
    ...
    getErrorResponse(singlePageUrl: any, reason: any) {
        return {
            url: () => singlePageUrl,
            status: () => 0,
            statusText: () => reason,
    ...
    async isAllowedByRobotsTxt(singlePageUrl: any, userAgent: any) {
        ...
        return await this.robotsChecker.isAllowed(singlePageUrl, userAgent)
            .catch((ex: any) => this.emit('error', `cannot evaluate robots.txt related to url: ${singlePageUrl}, exception: ${ex.toString()}`));
    }
