...
class Arachnid extends Events {
  ...
  markItemAsProcessed(item) {
    const maxLinksReached = this.maxResultsNum && (this.pagesProcessed.size >= this.maxResultsNum);
    if (!maxLinksReached) {
  ...
  addChildrenToQueue(extractedInfo, depth) {
    ...
    let i = 0;
    while (depthInLimit && resultsNumInLimit && i < extractedInfo.links.length) {
