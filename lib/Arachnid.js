...
class Arachnid extends Events {
  ...
  async traverse() {
    ...
    while (!this.pagesToVisitQ.isEmpty()) {
      this.emit("Getting next batch size from queue to process", {queueSize: pagesToVisitQ.size()});
  ...
  shouldAddToQueue(currentPageUrl) {
    //TODO: amend the condition to take into account subdomain links
    //TODO: check that pages are non-broken only if external and add to the results map
    const isSameHost = currentPageUrl.host === this.mainPageUrl.host;
    if (!isSameHost) {
      this.emit("pageCrawlingSkipped", {link: currentPageUrl, reason: "External Url"});
    }
    return isSameHost;
  }
