...
const crawlePage = async (browser, domain) => {
...
const Arachnid = async (domain, maxDepth, args) => {
  ...
  while(!pagesToVisitQ.isEmpty()) {
      ...
      const {response, extractedInfo} = await crawlePage(browser, currentPage.url);
      if(currentPage.depth < maxDepth) {
        extractedInfo.links.forEach(link => {
            pagesToVisitQ.enqueue({url: link, depth: currentPage.depth+1});
        });
      } else {
