...
class Arachnid extends Events {
  ...
  async processPageBatch(pagesToVisit) {
    ...
    await Promise.all(crawlPromises).then(allPagesData => {
      allPagesData.forEach(data => {
        const { url, extractedInfo, depth, headers, statusCode, statusText } = data;
        let pageInfoResult = {
          url,
  ...
  extractIndexability(pageInfoResult) {
    ...
    if (pageInfoResult.robotsHeader &&
      ...
      indexabilityStatus = pageInfoResult.statusText;
        ...
        encodeURI(pageInfoResult.canonicalUrl).toLowerCase() !== pageInfoResult.url.toLowerCase()){
        isIndexable = false;
  ...
  async crawlPage(browser, singlePageLink) {
    ...
    try {
      ...
      response = await page.goto(this.getNormalizedLink(singlePageLink.url), { waitUntil: 'domcontentloaded', timeout: 0 });
    } catch (ex) {
      response = {
        url: () => singlePageUrl,
        status: () => 0,
    ...
    if (response.status() > 399 || response.status() == 0) {
      ...
      if (this.shouldExtractInfo(singlePageLink.url, response)) {
        try {
          ...
          this.emit('error', `cannot extract info for ${singlePageUrl}, reason: ${ex.toString()}`);
        }
      }
    }
    page.close();
