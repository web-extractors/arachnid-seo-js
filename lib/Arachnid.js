...
class Arachnid extends Events {
   ...
   * @param {number} maxResultsNum - set maximum pages to be traversed/returned
   */
  ...
  async traverse() {
    ...
    queueLoop: while (!this.urlsToVisitQ.isEmpty()) {
      ...
      for (let i = 0; i < pagesInfoResults.length; i++) {
        ...
        if (this.maxResultsNum && this.currInternalLinkCount >= this.maxResultsNum) {
  ...
  markItemAsProcessed(item) {
    const maxLinksReached = item.maxResultsNum && (this.currInternalLinkCount >= this.maxResultsNum);
    if (!maxLinksReached && !this.pagesProcessed.has(item.url)) {
      this.pagesProcessed.set(item.url, item);
      if (item.isInternal) {
        this.currInternalLinkCount++;
      }
  ...
  addChildrenToQueue(extractedInfo, depth) {
    if (!this.maxDepth || depth < this.maxDepth) {
      ...
      this.emit('info', `reached max depth ${this.maxDepth}, not adding more links of depth ${depth}`);
    }
  }
  ...
  shouldExtractInfo(currentPageUrl) {
    let shouldFollow = false;
    if (this.followSubDomains && this.isSubDomain(currentPageUrl)) {
      shouldFollow = true;
    } else if (this.isSameHost(currentPageUrl)) {
      shouldFollow = true;
    } else {
  ...
  async crawlPage(browser, singlePageLink) {
    ...
    page.on('response', async (response) => {
      ...
      if (redirectChain.includes(response.url())) {
