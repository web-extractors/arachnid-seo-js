...
class Arachnid extends Events {
  ...
  async traverse() {
    ...
    while (!this.pagesToVisitQ.isEmpty()) {
      this.emit('info', `Getting next batch size from queue to process, current queue size ${this.pagesToVisitQ.size()}`);
      ...
      extractedInfo.links.forEach(link => {
        try {
          ...
          if (this.shouldAddToQueue(extractedUrl)) {
            this.pagesToVisitQ.enqueue({ url: extractedUrl, depth: depth + 1 });
          }
        } catch (ex) {
