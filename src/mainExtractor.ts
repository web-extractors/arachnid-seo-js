...
const extractSelectorContents = async (page: Page, selector: string): Promise<string[]> => new Promise((resolve, reject) => {
  try {
    resolve(extractElemContents(page, selector))
  } catch (error: unknown) {
    reject(new Error(`Failed to extract contents for selector ${selector}`))
  }
})

const extractMeta = async (page: Page): Promise<any> => new Promise((resolve, reject) => {
  try {
    resolve(
      page.evaluate(() => [...document.querySelectorAll('meta')]
        .filter((element) => {
...
const extractImages = async (page: Page): Promise<ImageInfo> => {
  return new Promise<ImageInfo>((resolve, reject) => {
    page.evaluate(() =>
      ...
      resolve({ missingAlt: [...new Set(images)] })
    }).catch((error) => {
      reject(error)
    })
  })
...
const extractElemContents = async (page: Page, elemSelector: string): Promise<string[]> => await page.evaluate(
  (selector: string) => [...document.querySelectorAll(selector)].map((elem) => (elem.textContent !== null ? elem.textContent.trim() : '')),
  elemSelector
)

const extractCanonical = async (page: Page): Promise<string> => await page.evaluate(() => document.querySelector("link[rel='canonical']")?.getAttribute('href') ?? '')
