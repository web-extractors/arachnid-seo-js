...
export default class Arachnid extends Events.EventEmitter {
   ...
   */
  setPuppeteerParameters(parameters: T[]) {
  ...
  _isValidHttpUrl(URLToCheck: string) {
    let url;
  ...
  async crawlPage(browser: any, singlePageLink: Page) {
    ...
    page.on('response', (emittedResponse: any) => {
      const status = emittedResponse.status();
      if (status >= 300 && status <= 399) {
        const responseUrl = new URL(emittedResponse.url());
        const pageUrl = this.getNormalizedLink(responseUrl);
        this.markItemAsProcessed({
          url: pageUrl,
          statusCode: status,
          statusText: emittedResponse.statusText(),
          /* tslint:disable:no-string-literal */
          redirectUrl: emittedResponse.headers()['location'],
          contentType: emittedResponse.headers()['content-type'],
          /* tslint:disable:no-string-literal */
          isInternal: this.isInternalLink(responseUrl),
        });
      }
    });
    const response = await page.goto(this.getNormalizedLink(singlePageLink.url), {
