...
import { CrawlPageResult, IndexabilityInfo, ResultInfo } from './types/arachnid'
...
export default class Arachnid extends EventEmitter {
  ...
  public async traverse (): Promise<Map<string, ResultInfo>> {
    ...
    let exitQueueLoop = false
    while (!this.urlsToVisitQ.isEmpty() && !exitQueueLoop) {
      this.emit(
        'info',
    ...
    await Promise.all(crawlPromises)
      .then((allPagesData) => {
        allPagesData.forEach((data: CrawlPageResult) => {
          const {
            link, response, extractedInfo, depth, resourceInfo, responseTimeMs
          } = data
  ...
  private addChildrenToQueue (extractedInfo: ExtractedInfo, depth: number, link: Link) {
    ...
    if (!depthInLimit) return

    let i = 0
    while (extractedInfo?.links && i < extractedInfo.links.length) {
      const urlString = extractedInfo.links[i++]
      if (this.pagesProcessed.has(urlString)) {
        continue
      }
      const resultsNumInLimit = !this.maxResultsNum || this.pagesProcessed.size + this.urlsToVisitQ.size() < this.maxResultsNum
      ...
      try {
        ...
        this.emit('pageUrlQueued', { url: url.href, depth: depth + 1 })
      } catch (ex: any) {
