...
import {
  CrawlPageResult, ErrorResponse, ResultInfo, PageResourceType
} from './types/arachnid'
...
export default class PageCrawler extends EventEmitter {
    ...
    try {
      ...
      return {
        ...
        response: this.getErrorResponse(
          singlePageLink.getNormalizedLink(),
          "Couldn't crawl link, unable to resolve host or connectivity issue"
        )
      }
    }
  }
  ...
  public async doCrawlPage (singlePageLink: Link, browser: Browser): Promise<CrawlPageResult> {
    ...
    page.on('response', async (subResponse: HTTPResponse) => {
      if ([301, 302].includes(subResponse.status()) && redirectChain.includes(subResponse.url())) {
        ...
        if (!subrequestRobotsAllowed) {
          this.markResponseAsVisited(this.getErrorResponse(subResponse.url(), 'Blocked by robots.txt'), singlePageLink)
        } else {
    ...
    const response: HTTPResponse | ErrorResponse | null = await page
      ...
      .catch((error: Error) => {
        ...
        } if (error?.stack?.includes('ERR_CONNECTION_REFUSED')) {
          return this.getErrorResponse(singlePageUrl, 'Connection refused')
        } if (error?.stack?.includes('net::ERR_INTERNET_DISCONNECTED')) {
    ...
    const resultItem: CrawlPageResult = {
      link: singlePageLink,
      response: response || this.getErrorResponse(singlePageUrl, 'Unknown error'),
      extractedInfo,
      depth: singlePageLink.getDepth(),
      responseTimeMs:
        pagePerformanceEntry.length > 0
          ? Math.floor(pagePerformanceEntry[0].responseStart - pagePerformanceEntry[0].requestStart)
          : undefined
    }
  ...
  private async isAllowedByRobotsTxt (pageLink: string, userAgent: string): Promise<boolean> {
    ...
    return await this.robotsChecker
      ...
      .catch((ex: Error) => this.emit('error', `cannot evaluate robots.txt related to url: ${pageLink}, exception: ${ex.toString()}`))
  }

  private recordSubResource (subResponse: HTTPResponse, resourcesMap: Map<string, PageResourceType>) {
  ...
  private shouldExtractInfo (pageLink: Link, response: HTTPResponse | ErrorResponse): boolean {
    if (response.headers()['content-type'] && !response.headers()['content-type'].includes('text/html')) {
      ...
      return false
    } if (this.followSubDomains && pageLink.isSubDomain()) {
      return true
    } if (pageLink.isSameHost()) {
      return true
    }
    this.emit('pageCrawlingSkipped', {
      url: decodeURI(pageLink.getPageUrl().toString()),
  ...
  private markResponseAsVisited (response: HTTPResponse | ErrorResponse, originalLink: Link) {
